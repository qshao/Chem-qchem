...
from qchem_gnn.graph import GraphBatch, GraphData, build_graph_from_smiles
...
def extract_intermediate_embeddings(
    ...
    """
    Run frozen GNN and collect pooled graph embeddings at every layer.
    ...
    layer_embs = [np.concatenate(chunks) for chunks in per_layer]
...
class EngineAdapterHead(nn.Module):
    """
    Tunable side structure adapted from ENGINE (Zhu et al.).

    For L frozen GNN message-passing layers, maintains a side embedding h
    that accumulates information layer-by-layer via learnable alpha gates:

        h_0 = zeros
        h_i = proj_i(gnn_state_i) * σ(α_i) + h_{i-1} * (1 - σ(α_i))
        exit_pred_i = head_i(h_i)

    All GNN weights are frozen. Only projections, alphas, and exit heads
    are trained, with loss summed across every exit:

        L = Σ_i MSE(exit_pred_i, y)

    This gives each layer's head a direct gradient signal and lets the model
    learn how much to trust each layer (via α_i) rather than committing to
    one representation depth.
    """
    ...
    def __init__(self, hidden_dim: int, num_layers: int):
        ...
        # Initialise alphas at 0 → sigmoid(0)=0.5 equal blend to start
    ...
    def forward(self, layer_tensors: list[torch.Tensor]) -> list[torch.Tensor]:
        """Return one prediction tensor [B, 1] per layer."""
    ...
    def predict_ensemble(self, layer_tensors: list[torch.Tensor]) -> np.ndarray:
        """Average prediction across all exit heads."""
    ...
    def predict_early_exit(
        ...
        """
        Early exit: stop each molecule once prediction std across accumulated
        exit heads drops below `tolerance`. Molecules that never converge use
        the final layer's prediction.
        ...
        for i, (proj, alpha, head, x) in enumerate(
            ...
            gate = torch.sigmoid(alpha)
            h = proj(x) * gate + h * (1.0 - gate)
            pred = head(h).squeeze(1)           # [B]
            accumulated.append(pred)

            if i >= min_layers and len(accumulated) >= 2:
                stack = torch.stack(accumulated, dim=0)  # [i+1, B]
                std = stack.std(dim=0)                   # [B]
